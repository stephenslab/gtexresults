...
hist((het.func((ebnorm,threshold=0.2)),main="Number of Tissues in which B.norm<alpha*MaxB",xlab="NumberofTissues",nclass=50)
...
lines(table(het.func(ebnorm,threshold=0.6)),col="purple")
#lines(table(het.func(ebnorm,threshold=0.7)),col="orange")
legend("center",col=c("black","green","blue","red","purple"),legend=c("alpha=0.2","alpha=0.3","alpha=0.4","alpha=0.5","alpha=0.6"),lty=1)
```
...
sigmat=(lfsr<=thresh)
nsig= rowSums(sigmat)
 hist(nsig[nsig>0],main="Number of tissues in which eQTl significant", nclass=45,xlab="Number of tissues",freq=TRUE)
```

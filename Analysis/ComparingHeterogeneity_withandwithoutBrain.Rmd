...
hist((het.func(het.norm(effectsize=pm.mash.beta),threshold=0.5)),main="Number of Tissues in which B.norm>0.5*MaxBEXCLUDINGBRAIN",xlab="NumberofTissues",nclass=40,col="blue",freq=FALSE)
...
hist(sign.tissue.func(het.norm(effectsize=pm.mash.beta.brain))/44,main="Number of Tissues in which SignDifferentThanMax,ALL",xlab="NumberofTissues",nclass=50,col="blue",freq=FALSE,ylim=c(0,4))
hist(sign.tissue.func(het.norm(effectsize=pm.mash.beta))/34,main="Number of Tissues in which SignDifferentThanMax, EXCLUDING brain",xlab="NumberofTissues",nclass=40,col="blue",freq=FALSE,ylim=c(0,4))
hist(sign.tissue.func(het.norm(effectsize=pm.mash.beta.brain[,c(7:16)]))/10,main="Number of Tissues in which SignDifferentThanMax,brain",xlab="NumberofTissues",nclass=40,col="blue",freq=FALSE,ylim=c(0,4))
...
Now, let's consider the number of tissues vs normalziing value. We plot the median of the value used to normalize for each group of SNPs that are of the same number of tissue class.
...
Let's compare the empirical distribtuion with and wihtout brains of the proportion of tissues which are greater than 50% of maximum effect:

```{r prop.max.effect.ecdf}
require( Hmisc )
...
```{r echo=FALSE}
normalized.nobrain=het.norm(pm.mash.beta)
normalized.all=het.norm(pm.mash.beta.brain)
normalized.brainonly=het.norm(pm.mash.beta.brain[,c(7:16)])

##Turn negs to positives##
negs=which(normalized.all<0);normalized.all[negs]=0
negs=which(normalized.nobrain<0);normalized.nobrain[negs]=0
negs=which(normalized.brainonly<0);normalized.brainonly[negs]=0


##Compute sum for only those normalized effects that are not negative
hetindex.nobrain=apply(normalized.nobrain,1,function(x){
  sum(x[x>0])
})
hetindex=apply(normalized.all,1,function(x){
  sum(x[x>0])
})

hetindex.brain.only=apply(normalized.brainonly,1,function(x){
  sum(x[x>0])
})

# par(mfrow=c(1,2))
# ma=apply(abs(pm.mash.beta),1,function(x){
#   abs(median(x)-mean(x))
# })
# hist(ma,nclass=100)
# ma=apply(normalized.all,1,function(x){
#   abs(median(x)-mean(x))
# # })
# 
# # hist(manob,nclass=100,freq=FALSE,col="green")
# # 
#  man.nobrain=apply(normalized.nobrain,1,function(x){
#    abs(abs(median(x))-abs(mean(x)))
#  })
# # 
# 
#  man.alltissues=apply(normalized.all,1,function(x){
#    abs(abs(median(x))-abs(mean(x)))
#  })
# 
# 
#  man.brainonly=apply(normalized.brainonly,1,function(x){
#    abs(abs(median(x))-abs(mean(x)))
#  })
# 
# hist(man.nobrain,nclass=100,freq=FALSE,col="green",main="Hist of abs(Median-Average)")
# lines(density(man.alltissues),col="red")
# lines(density(man.brainonly),col="blue")
# 
# 
# legend("topright",col=c("green","red"),legend=c("ExcludingBrain","AllTissues"),lty=1)

par(mfrow=c(1,3));
hist(hetindex,main="Histogram of Sum of (Normalized Bjr),AllTissues",xlab="Homogeneity Index",col="blue",freq=F);
hist(hetindex.nobrain,main="Histogram of Sum of (Normalized Bjr),NoBrain",xlab="Homogeneity Index",col="blue",freq=F)
hist(hetindex.brain.only,main="Histogram of Sum of (Normalized Bjr),BrainONLY",xlab="Homogeneity Index",col="blue",freq=F)

# par(mfrow=c(1,3))
# 
# hist(rowSums(het.norm(abs(pm.mash.beta.brain))),main="Histogram of Sum of (Normalized Bjr^2),ALL",xlab="Homogeneity Index",col="blue",freq=F)
# hist(rowSums(het.norm(abs(pm.mash.beta))),main="Histogram of Sum of (Normalized Bjr^2),NOBRAIN",xlab="Homogeneity Index",col="blue",freq=F)
# hist(rowSums(het.norm(abs(pm.mash.beta.brain[,c(7:16)]))),main="Histogram of Sum of (Normalized Bjr^2),BrainOnly",xlab="Homogeneity Index",col="blue",freq=F)

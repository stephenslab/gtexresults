...
sigmat=(lfsr<=thresh)
nsig= rowSums(sigmat)
hist(nsig[nsig>0], nclass=45,xlab="Number of tissues",col="blue",main="Number of Tissues Significant")
#mtext("AllTissues")
...
hist((het.func(het.norm(effectsize=pm.mash.beta),threshold=0.5)),main="Number of Tissues >50% Max Effect",xlab="NumberofTissues",nclass=50,col="blue",freq=FALSE)#,ylim=c(0,0.6))
#mtext("All Tissues")
...
hist((het.func(het.norm(effectsize=pm.mash.beta[,-c(7:16)]),threshold=0.5)),main="",xlab="NumberofTissues",nclass=40,col="blue",freq=FALSE,ylim=c(0,0.6))
mtext("Excluding Brain")

brain.norm=het.norm(effectsize=pm.mash.beta[,c(7:16)])
hist(het.func(brain.norm,threshold=0.5),main="",xlab="NumberofTissues",nclass=12,col="blue",freq=FALSE,ylim=c(0,0.6))
mtext("BrainOnly")
...
(signall.brainonly=mean(het.norm(pm.mash.beta[,c(7:16)])>0))
...
##show that results are robust###
(magall.excludingbrain=mean(het.norm(pm.mash.beta[,-c(7:16)])>0.5))
(magall.brainonly=mean(het.norm(pm.mash.beta[,c(7:16)])>0.5))

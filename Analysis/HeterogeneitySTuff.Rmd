...
nsig= rowSums(sigmat)
hist(nsig[nsig>0],main="",xlab="NumberofTissues",nclass=45,col="grey",freq=FALSE)
...
nsig= rowSums(sigmat)
hist(nsig[nsig>0],main="",xlab="NumberofTissues",breaks=seq(0,10,by = 1),col="grey",freq=FALSE)
#barplot(table(nsig[nsig>0]),xlab="Number of tissues",main="")
...
hist((het.func(het.norm(effectsize=pm.mash.beta[nsig>0,]),threshold=0.5)),main="Number of Tissues >50% Max Effect",xlab="NumberofTissues",nclass=50,col="grey",freq=FALSE)#,ylim=c(0,0.6))
#mtext("All Tissues")
...
par(oma=c(3,3,3,3) )
par(mfrow=c(1,3))
hist((het.func(het.norm(effectsize=pm.mash.beta[nsig>0,]),threshold=0.5)),main="",xlab="NumberofTissues",col="grey",freq=FALSE,nclass=50)#,ylim=c(0,0.6))
# 
...
sigmat=(lfsr.nobrain<=thresh)
nsig= rowSums(sigmat)
hist((het.func(het.norm(effectsize=pm.mash.nobrain[nsig>0,]),threshold=0.5)),main="",xlab="NumberofTissues",nclass=35,col="grey",freq=FALSE)#,ylim=c(0,0.6))
...
hist((het.func(het.norm(effectsize=pm.mash.beta[nsig>0,]),threshold=0.5)),main="",xlab="NumberofTissues",nclass=50,col="grey",freq=FALSE)#,ylim=c(0,0.6))
mtext("All Tissues")
...
hist((het.func(het.norm(effectsize=pm.mash.beta[nsig>0,-c(7:16)]),threshold=0.5)),main="",xlab="NumberofTissues",nclass=40,col="grey",freq=FALSE)#,ylim=c(0,0.6))
mtext("Excluding Brain")
...
Let's also consider how our power changes when we perform the separate analyses:

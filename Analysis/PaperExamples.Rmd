...
barplot(as.numeric(z.stat[j,]),col = as.character(gtex.colors),las=2,main=paste0("Z.mle",title))#,ylim= c(xmins-0.1, xmax + 0.1))
x=as.numeric(posterior.betas[j,])
xmins=min(x)
xmax=max(x)
bp <- barplot(x, ylim = c(xmins-0.1, xmax + 0.1),col = as.character(gtex.colors),las=2,main=paste0("E(B|Data)",title),names=abr.names,cex.names=0.5)
text(bp, x, 

...
sigmat=lfsr.all<0.05
rm(lfsr.brainonly)
rm(lfsr.nobrain)

##now restrict to same sign and significant
thresh=0.05
post.means=read.table("../Data/Aug13withEDposterior.means.txt")[,-1]
post.means=post.means[rowSums(sigmat)>0,]
shared=matrix(NA,nrow = ncol(lfsr.mash),ncol=ncol(lfsr.mash))
for(i in 1:ncol(lfsr.mash)){
  for(j in 1:ncol(lfsr.mash)){
    sig.row=which(lfsr.mash[,i]<thresh)
...
exclude.brain.order=read.table("../../Dropbox/excludebrainorder.txt")[,1]
all.tissue.order=read.table("../../Dropbox/alltissueorder.txt")[,1]
##make sure ordering right

##now restrict to the same sign and intersect

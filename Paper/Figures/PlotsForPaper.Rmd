...
legend("bottom",legend=c("MASH","BMALite","ASH"),pch=c(2,1,3),col=c("blue","red","green"))
title("True eQTL Called vs Significance (LFSR)")


##plot signs correctly
...
for(thresh in seq(from = 0.01,to = 0.1,by = 0.001)){
points(thresh,sum(sign.test.mash>0&lfsr.mash<thresh),col="blue",pch=1)}
for(thresh in seq(from = 0.01,to = 0.1,by = 0.001)){
points(thresh,sum(sign.test.bma>0&lfsr.bma<thresh),col="red",pch=2)}
for(thresh in seq(from = 0.01,to = 0.1,by = 0.001)){
points(thresh,sum(sign.test.ash>0&lfsr.ash<thresh),col="green",pch=3)}
...
plot(as.matrix(beta[1:10000,]),as.matrix(post.means)*sebetahat[1:10000,],col=adjustcolor("blue", alpha=0.5), pch=16,main="E(B|Data_mash) vs B.true",ylab="E(B|Data)",xlab="B.truth",xlim=c(-2,2),ylim=c(-2,2))
...
plot(as.matrix(beta[1:10000,]),as.matrix(univariate.ash.pm)*sebetahat[1:10000,],col=adjustcolor("blue", alpha=0.5), pch=16,main="E(B|Data.ash) vs B.true",ylab="E(B|Data)",xlab="B.True",xlim=c(-2,2),ylim=c(-2,2))
...
barplot(colSums(pi.mat),las=2,names=c(as.matrix(abr.names[,2]),"all"),main="mash.litePriorWeight",cex.names = 0.5)
mtext("Likelihood= −1,298,672") 

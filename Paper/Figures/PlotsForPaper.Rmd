...
plot(unlist(as.vector(maxz)),unlist(as.vector(pm.ash)),col=rgb(0,100,0,50,maxColorValue=255), pch=16,main="SignificancevsZ_ASH",xlab="Z",ylab="ASH__E(B|D)*sj")
plot(unlist(as.vector(maxz)),unlist(as.vector(pm.mash)),col=rgb(0,100,0,50,maxColorValue=255), pch=16,main="E(Z|D)vsZ.hat_MASH",xlab="Z",ylab="MASH__E(B|D)*sj")
dev.off()
```
...
a=barplot(as.numeric(z.stat[t,]),las=2,cex.names=0.5,names=colnames(z.stat))
title(main=paste0("Z Statistic"),font.main=1,col.main="blue")
mtext(paste0(name),col="purple")
b=barplot(as.numeric(posterior.means[t,]),col=col.func(t,lfsr=lfsr,posterior.means=posterior.means),cex.names=0.5,las=2,ylab="E(Z|Data)",names=colnames(posterior.means))
...
eqtlbma.confg=c(rep(0,43),1)
barplot(eqtlbma.confg,main="SingletonConfig")
...
cons.config=t(as.matrix(rep(1,44)));colnames(cons.config)=colnames(maxz)
barplot(cons.config,las=2,main=paste0("mash.lite.consistent.config"),las=2,cex.names=0.5)

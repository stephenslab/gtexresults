...
legend("right",legend=c("Tissue_specific","TrueNulls"),pch=1,col=c("green","red"))
dev.off()
...
plot(unlist(as.vector(maxz)),unlist(as.vector(pm.ash)),col=rgb(0,100,0,50,maxColorValue=255), pch=16,main="E(Z|D)vsZ_ASH",xlab="Z",ylab="ASH__E(Z|D)")
plot(unlist(as.vector(maxz)),unlist(as.vector(pm.mash)),col=rgb(0,100,0,50,maxColorValue=255), pch=16,main="E(Z|D)vsZ.hat_MASH",xlab="Z",ylab="MASH_E(Z|D)")
dev.off()
```
...
barplot(colSums(pi.mat),las=2,names=c(colnames(maxz),"all"),main="mash.litePriorWeight",cex.names = 0.5)
...
v=svd(covmat[[i]])$v
  colnames(v)=rownames(v)=colnames(maxz)
max.effect=sign(v[,1][which.max(abs(v[,1]))])
barplot(max.effect*v[,1],las=2,main=paste0("EigenVector1ofUk=",i),col=i-1,axisnames=ifelse(i==7,TRUE,FALSE),cex.names=ifelse(i==7,0.5,NULL))
}
```

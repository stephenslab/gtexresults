...
png("scatterplot.fittedtspec.ash.png")
...
![alt text](scatterplot.fittedtspec.ash.png)
...
plot(unlist(as.vector(maxb)),unlist(as.vector(pm.ash.beta)),col=rgb(0,100,0,50,maxColorValue=255), pch=16,main="UnivariateAshAppliedtoAllTissues",xlab="B.mle",ylab="UnivariateAshE(B|D)")
plot(unlist(as.vector(maxb)),unlist(as.vector(pm.mash.beta)),col=rgb(0,100,0,50,maxColorValue=255), pch=16,main="MatrixAshPosteriorMeansvsUnivariateSumStats",xlab="B.mle",ylab="MatrixAshE(B|D)")
dev.off()
...
plot(unlist(as.vector(maxz)),unlist(as.vector(pm.ash)),col=rgb(0,100,0,50,maxColorValue=255), pch=16,main="Z.mle",xlab="SummaryUnivariateZStats",ylab="UnivariateAshE(Z|D)")
plot(unlist(as.vector(maxz)),unlist(as.vector(pm.mash)),col=rgb(0,100,0,50,maxColorValue=255), pch=16,main="MatrixAshPosteriorMeansvsUnivariateSumStats",xlab="Z.mle",ylab="MatrixAshE(Z|D)")
dev.off()
```
...
Significant b_{jr} ≤ 0.05 393,414           91,755 401,552
```
...
```{r,echo=FALSE}
par(mfrow=c(1,2))
pi.bma=readRDS("~/Dropbox/pisbmaonly.rds")$pihat
pi.mat=matrix(data = pi.bma,nrow = 22,ncol =45,byrow = TRUE)
barplot(colSums(pi.mat),xlab="Non0Tissue",las=2,names=c(colnames(maxz),"all"),main="BMAlitePriorWeight",cex.names = 0.5)

pi.mash=readRDS("../../Data/pisAug13withED.rds")$pihat
pi.mat.mash=matrix(data = pi.mash,nrow = 22,ncol =54,byrow = TRUE)
barplot(colSums(pi.mat.mash),xlab="Uk",las=2,names=c(paste0("Uk",seq(1:9)),colnames(maxz),"all"),main="MASHPriorWeight",cex.names = 0.5)
```
...
a=barplot(as.numeric(z.stat[t,]),las=2,cex.names=0.5,main=paste0("Z Statistics",name),names=colnames(z.stat))
b=barplot(as.numeric(posterior.means[t,]),main=paste0("E(Z|EZ)",name),col=col.func(t,lfsr=lfsr,posterior.means=posterior.means),
          cex.names=0.5,las=2,ylab="E(Z|Data)",names=colnames(posterior.means))
}
...
```
 ...
 Lastly, the inclusion of the eQTLBMA lite configurations (in which the SNP has a 378 non-zero effect in only one tissue) coupled with the learned patterns of tissue specificity 379 evident in matrices Uk : 5 − 9 serve to allow the preservation of qualitatively specific 380 effects. Here, we show a gene-snp pair demonstrating high loading on one of the 381 eqtlbma-lite configuration matrices - indeed, we reject the significance of the effect size 382 estimates in all tissues but testes, a pattern consistent with the presence of 383 tissue-specificity described below. Together, these results cement the resolution afforded 384 by methods which can distinguish among tissues in which a QTL is called active, 385 beyond reducing genetic effects to binary ‘on’ or ‘off’ conclusions
```{r,echo=FALSE}
#newfunc(t=nine.ex.2)
newfunc(t=nine.ex.3)
```
...
#par(mfrow=c(1,2))
plot(abs(max.mash),het.norm(effectsize = pm.mash.beta.nobrain),main="NormalizingB.MASH",xlab="absNormalizingValue",ylab="E(B|data)_norm", 
...
par(mfrow=c(1,1))
plot(abs(max.mash.brain),het.norm(effectsize = pm.mash.beta.brain),main="NormalizingB.MASH",xlab="absNormalizingValue",ylab="E(B|data)_norm", 
     col=rgb(0,100,0,50,maxColorValue=255), pch=16,xlim=c(0,2))
#plot(abs(max.mle),bnorm,main="NormalizingB.MLE",ylab="B_norm",xlab="absNormalizingValue",col=rgb(0,100,0,50,maxColorValue=255), pch=16,xlim=c(0,2))
dev.off()

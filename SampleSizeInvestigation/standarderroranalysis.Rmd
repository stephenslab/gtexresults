...
Show that my results V(B|D)=E(E(B^2|D,K))-E(E((B|D,k)
are the same as V(B|D)=E(V(B|D,K))+V(E(B|D,k))
...
v1=post.weights%*%all.arrays$post.covs+post.weights%*%(t(apply((all.arrays$post.means),1,'-',(post.weights%*%all.arrays$post.means)))^2)

v2=post.weights%*%(all.arrays$post.covs+all.arrays$post.means^2)-(post.weights%*%all.arrays$post.means)^2

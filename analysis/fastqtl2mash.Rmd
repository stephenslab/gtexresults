...
that is more convenient for mash analysis. This code was used to
generate data file `MatrixEQTLSumStats.Portable.Z.rds` in the
[git repository][gtexresults] from the SNP-gene association statistics
included as part of Release 6 of the [GTEx Project][gtex]
...
Next, we will use the `fastqtl_to_mash.ipynb` source code in the
`workflows` directory to convert the toy data set in FastQTL format to
the mash format. The toy data are stored in the `data/fastqtl`
...
*Add information here about the expected outputs.*
...
options and other usage information.

+ In practice for GTEx data the conversion is computationally intensive
and is best done on a cluster environment with configurations to run
the workflow across different compute nodes. See

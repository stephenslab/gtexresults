...
And plot the posterior means vs the truth:

```{r}
nulls=which(b.truth.test[,1]=="0",)
null.vals=intersect(1:nrow(post.means),nulls)
post.means=as.matrix(post.means)
pmash=as.matrix(pmash)
par(mfrow=c(1,2))
plot(as.matrix(b.test),as.matrix(post.means),xlab="Bhat",ylab="Post.MeanswithMASH",main="Post.MeanvsMLE,MASH")
points(b.test[null.vals,],post.means[null.vals,],col="red")
plot(as.matrix(b.test),as.matrix(pmash),xlab="Bhat",ylab="Post.MeanswithASH",main="Post.MeanvsMLE,ASH")
points(b.test[null.vals,],as.matrix(pmash[null.vals,]),col="red")
```

```{r datalookslike}
hist(testdat$beta[1:80,],nclass=100,freq=FALSE,main="HistogramofTrueEffects")
lines(density(testdat$beta[81:1000,]),col="red")

hist(testdat$betahat[1:80,],nclass=100,freq=FALSE,main="HistogramofEstEffects")
lines(density(testdat$betahat[81:1000,]),col="red")
```